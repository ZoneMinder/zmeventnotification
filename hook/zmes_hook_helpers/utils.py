...
def rescale_polygons(xfactor, yfactor):
    ...
    g.logger.debug(
        'resized polygons x={}/y={}: {}'.format(xfactor, yfactor, newps))
    g.polygons = newps
...
def import_zm_zones(mid):
    ...
    g.logger.debug(
        'Getting ZM zones using {}?username=xxx&password=yyy'.format(url))
...
def download_files(args):
    ...
    if g.config['frame_id'] == 'bestmatch':
        ...
        filename1 = g.config['image_path'] + \
            '/' + args['eventid'] + '-alarm.jpg'
        filename1_bbox = g.config['image_path'] + \
            '/' + args['eventid'] + '-alarm-bbox.jpg'
        filename2 = g.config['image_path'] + \
            '/' + args['eventid'] + '-snapshot.jpg'
        filename2_bbox = g.config['image_path'] + \
            '/' + args['eventid'] + '-snapshot-bbox.jpg'

        url = g.config['portal'] + '/index.php?view=image&eid=' + args['eventid'] + \
            '&fid=alarm' + '&username=' + g.config['user'] + '&password=' + g.config['password']
        durl = g.config['portal'] + '/index.php?view=image&eid=' + args['eventid'] + \
            '&fid=alarm' + '&username=' + g.config['user'] + '&password=*****'
        ...
        url = g.config['portal'] + '/index.php?view=image&eid=' + args['eventid'] + \
            '&fid=snapshot' + '&username=' + g.config['user'] + '&password=' + g.config['password']
        durl = g.config['portal'] + '/index.php?view=image&eid=' + args['eventid'] + \
            '&fid=snapshot' + '&username=' + g.config['user'] + '&password=*****'
        g.logger.debug('Trying to download {}'.format(durl))
        try:
            input_file = opener.open(url)
        except HTTPError as e:
        ...
        filename1_bbox = g.config['image_path'] + \
            '/' + args['eventid'] + '-bbox.jpg'
        filename2 = ''
        filename2_bbox = ''
        url = g.config['portal'] + '/index.php?view=image&eid=' + args['eventid'] + '&fid=' + \
            g.config['frame_id'] + '&username=' + g.config['user'] + '&password=' + g.config['password']
        durl = g.config['portal'] + '/index.php?view=image&eid=' + args['eventid'] + \
            '&fid=snapshot' + '&username=' + g.config['user'] + '&password=*****'
        g.logger.debug('Trying to download {}'.format(durl))
        input_file = opener.open(url)
...
def process_config(args, ctx):
    ...
    try:
        ...
        g.config['basic_password'] = config_file['general'].get(
            'basic_password', '')
        g.config['image_path'] = config_file['general'].get(
            'image_path', '/var/lib/zmeventnotification/images')
        g.config['detect_pattern'] = config_file['general'].get(
            'detect_pattern', '.*')
        g.config['frame_id'] = config_file['general'].get(
            'frame_id', 'snapshot')
        g.config['resize'] = config_file['general'].get('resize', '800')
        g.config['delete_after_analyze'] = config_file['general'].get(
            'delete_after_analyze', 'no')
        g.config['show_percent'] = config_file['general'].get(
            'show_percent', 'no')
        g.config['log_level'] = config_file['general'].get('log_level', 'info')
        g.config['allow_self_signed'] = config_file['general'].get(
            'allow_self_signed', 'yes')
        g.config['write_image_to_zm'] = config_file['general'].get(
            'write_image_to_zm', 'yes')
        g.config['write_debug_image'] = config_file['general'].get(
            'write_debug_image', 'yes')
        g.config['models'] = str_split(
            config_file['general'].get(
                'models', 'yolo'))
        g.config['poly_color'] = eval(
            config_file['general'].get(
                'poly_color', '(127, 140, 141)'))

        # YOLO stuff
        g.config['config'] = config_file['yolo'].get(
            'config', '/var/lib/zmeventnotification/models/yolov3/yolov3.cfg')
        g.config['weights'] = config_file['yolo'].get(
            'weights', '/var/lib/zmeventnotification/models/yolov3/yolov3.weights')
        g.config['labels'] = config_file['yolo'].get(
            'labels', '/var/lib/zmeventnotification/models/yolov3/yolov3_classes.txt')

        g.config['tiny_config'] = config_file['yolo'].get(
            'tiny_config', '/var/lib/zmeventnotification/models/tinyyolo/yolov3-tiny.cfg')
        g.config['tiny_weights'] = config_file['yolo'].get(
            'tiny_weights', '/var/lib/zmeventnotification/models/tinyyolo/yolov3-tiny.weights')
        g.config['tiny_labels'] = config_file['yolo'].get(
            'tiny_labels', '/var/lib/zmeventnotification/models/tinyyolo/yolov33-tiny.txt')
        ...
        g.config['face_num_jitters'] = int(
            config_file['face'].get('num_jitters', '0'))
        g.config['face_upsample_times'] = int(
            config_file['face'].get('upsample_times', '1'))
        g.config['face_model'] = config_file['face'].get('model', 'hog')
        g.config['known_images_path'] = config_file['face'].get(
        ...
        if args['monitorid']:
            if config_file.has_option(
                'monitor-%s' %
                args['monitorid'],
                    'detect_pattern'):
                # local detect_pattern overrides global
                g.config['detect_pattern'] = config_file['monitor-%s' %
                                                         args['monitorid']].get('detect_pattern', '.*')
                g.logger.debug(
                    'monitor with ID {} has specific detection pattern: {}'.format(
                        args['monitorid'], g.config['detect_pattern']))
        ...
        # Check if we have a custom model sequence for the current monitor
            if config_file.has_option(
                ...
                g.config['models'] = str_split(
                    config_file['monitor-%s' % args['monitorid']].get('models', 'yolo'))
                g.logger.debug(
                    'monitor with ID {} has specific models: {}'.format(
                        args['monitorid'], g.config['models']))
        ...
        if args['monitorid']:
            if config_file.has_section('monitor-' + args['monitorid']):
                ...
                if itms:
                    g.logger.debug(
                        'object areas definition found for monitor:{}'.format(
                            args['monitorid']))
                else:
                    g.logger.debug(
                        'object areas section found, but no polygon entries found')
                ...
                g.logger.debug(
                    'no object areas found for monitor:{}'.format(
                        args['monitorid']))
        else:
            g.logger.info(
                'Ignoring object areas, as you did not provide a monitor id')
