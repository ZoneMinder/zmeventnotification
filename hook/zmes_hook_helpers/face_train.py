...
g.logger.debug(
    'Face Recognition library load time took: {} milliseconds'.format(diff_time))
    ...
    try:
        for entry in os.listdir(directory):
            if os.path.isdir(directory+'/'+entry):
                ...
                for person in person_dir:
                    if person.endswith(tuple(ext)):
                        ...
                        if len(face_locations) != 1:
                            g.logger.error(
                                'File {} has multiple faces, cannot use for training. Ignoring...'.format(person))
                        else:
                ...
                if len(face_locations) != 1:
                    g.logger.error(
                        'File {} has multiple faces, cannot use for training. Ignoring...'.format(entry))
                else:
