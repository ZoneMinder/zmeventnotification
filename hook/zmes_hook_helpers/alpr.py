...
class AlprBase:
    ...
    def prepare(self, object):
        if not isinstance(object, str):
            g.logger.debug(
                'Supplied object is not a file, assuming blob and creating file')
    ...
    def getscale(self):
        if g.config['resize'] != 'no':
            ...
            img_new = imutils.resize(img, width=min(
                int(g.config['resize']), img.shape[1]))
            ...
            img = None
            img_new = None
            g.logger.debug('ALPR will use {}x{} but Yolo uses {}x{} so ALPR boxes will be scaled {}x and {}y'.format(
                oldw, oldh, neww, newh, xfactor, yfactor))
        else:
            xfactor = 1
            yfactor = 1
...
class PlateRecognizer (AlprBase):
    def __init__(self, url=None, apikey=None, options={}, tempdir='/tmp'):
        ...
        g.logger.debug('PlateRecognizer ALPR initialized with options: {} and url: {}'.format(
            options, self.url))
    ...
    def detect(self, object):
        ...
        if options.get('stats') == 'yes':
            g.logger.debug('Plate Recognizer API usage stats: {}'.format(
        ...
        with open(self.filename, 'rb') as fp:
            try:
                ...
                response = {
                    'error': 'Plate recognizer rejected the upload. You either have a bad API key or a bad image', 'results': []}
                g.logger.debug(
        ...
        for plates in response['results']:
            ...
            if dscore >= options.get('min_dscore') and score >= options.get('min_score'):
                ...
                g.logger.debug('ALPR: discarding plate:{} because its dscore:{}/score:{} are not in range of configured dscore:{} score:{}'.format(
                    label, dscore, score, options.get('min_dscore'), options.get('min_score')))
        ...
        with open(self.filename, 'rb') as fp:
            try:
                ...
                if options.get('country'):
                    params = params + '&country='+options.get('country')
                if options.get('state'):
                    params = params + '&state='+options.get('state')
                if options.get('recognize_vehicle'):
                    params = params + '&recognize_vehicle=' + \
                        str(options.get('recognize_vehicle'))

                rurl = '{}?secret_key={}{}'.format(
                    self.url, self.apikey, params)
                g.logger.debug('Trying OpenALPR with url:' + rurl)
                response = requests.post(rurl, files={'image': fp})
            except requests.exceptions.RequestException as e:
                response = {
                    'error': 'Open ALPR rejected the upload. You either have a bad API key or a bad image', 'results': []}
                g.logger.debug(
        ...
        if response.get('results'):
            for plates in response.get('results'):
                ...
                if conf < options.get('min_confidence'):
                    g.logger.debug('OpenALPR: discarding plate: {} because detected confidence {} is less than configured min confidence: {}'.format(
                        label, conf, options.get('min_confidence')))
                    continue
