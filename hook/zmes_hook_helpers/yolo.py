...
class Yolo:
    ...
    def detect(self, image):
        ...
        g.logger.debug(
            '|---------- YOLO (input image: {}w*{}h, resized to: {}w*{}h) ----------|'.format(Width, Height, modelW, modelH))
        ...
        if self.initialize:
            ...
            g.logger.debug(
                'YOLO initialization (loading model from disk) took: {} milliseconds'.format(diff_time))
        ...
        blob = cv2.dnn.blobFromImage(
            image, scale, (modelW, modelH), (0, 0, 0), True, crop=False)
        ...
        for i in indices:
            ...
            if confidences[i] >= g.config['yolo_min_confidence']:
                ...
                g.logger.info('object:{} at {} has a acceptable confidence:{} compared to min confidence of: {}, adding'.format(
                    label[-1], bbox[-1], conf[-1], g.config['yolo_min_confidence']))
            else:
                g.logger.info('rejecting object:{} at {} because its confidence is :{} compared to min confidence of: {}'.format(str(self.classes[class_ids[i]]), [
                              int(round(x)), int(round(y)), int(round(x + w)), int(round(y + h))], confidences[i], g.config['yolo_min_confidence']))
