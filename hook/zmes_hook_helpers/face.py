...
try:
    ...
    raise ValueError(
        'Could not import face_recognition. Make sure you did sudo pip install face_recognition')
...
class Face:
    ...
    def __init__(self, upsample_times=1, num_jitters=0, model='hog'):
        g.logger.debug(
            'Initializing face recognition with model:{} upsample:{}, jitters:{}' .format(
                model, upsample_times, num_jitters))
            ...
            for filename in os.listdir(directory):
                if filename.endswith(tuple(ext)):
                    ...
                    if not len(known_image):
                        g.logger.debug(
                            'Skipping {} No faces detected'.format(filename))
                    else:
    ...
    def detect(self, image):
        ...
        face_locations = face_recognition.face_locations(
            rgb_image, model=self.model, number_of_times_to_upsample=self.upsample_times)
