...
def dict_to_yaml(yaml_file, dict_in):
    try:
        ...
        print(
            f"Exception while trying to create empty file using mode={_mode}. "
...
if __name__ == "__main__":
    ...
    if config_data.get("hook", {}).get("max_parallel_hooks"):
        print(
            f"This is assumed to be the ZMES Perl daemon configuration file (Default: ZMEventnotification.ini) "
            f"writing output to {yaml_file}"
        )
        _parse = True
    elif config_data.get("secret") or config_data.get("secrets"):
        print(
            f"This is assumed to be the secrets file for the Perl daemon (Default: secrets.ini)"
        )
        _parse = True
    elif config_data.get("ml", {}).get("stream_sequence") or config_data.get("ZMES"):
        print(
            f"This is assumed to be the object detection 'hook' configuration file (Default: objectconfig.ini)"
        )
        print(
        ...
        config_data.get("MLAPI_SECRET_KEY")
        or config_data.get("MLAPI")
        or config_data.get("zmes_keys")
    ):
        print(
            f"This is assumed to be a configuration or secrets file associated with MLAPI"
        )
        print(
