...
if [[ -z "$mid" ]]; then
      DETECTION_SCRIPT=("${zmes_dir}/bin/zm_detect.py" --eventid "$eid" --config "${config_file}"  --reason "${reason}"  --eventpath "${eventpath}" "$docker" "$bare_debug" "$debug" "$live")
elif [[ -n "$mid" ]]; then
       DETECTION_SCRIPT=("${zmes_dir}/bin/zm_detect.py" --monitor-id "$mid" --eventid "$eid" --config "${config_file}"  --reason "${reason}"  --eventpath "${eventpath}" "$docker" "$bare_debug" "$debug" "$live")
fi
