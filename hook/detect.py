...
if not args['file']:
    filename_alarm, filename_snapshot, filename_alarm_bbox, filename_snapshot_bbox = utils.download_files(
        args)
    # filename_alarm will be the first frame to analyze (typically alarm)
    # filename_snapshot will be the second frame to analyze only if the first
    # fails (typically snapshot)
else:
...
for filename in [filename_alarm, filename_snapshot]:
    ...
    if image is None:
        g.logger.error(
            'Error reading {}. It either does not exist or is invalid'.format(filename))
        raise ValueError(
            'Error reading file {}. It either does not exist or is invalid'.format(filename))

    oldh, oldw = image.shape[:2]

    g.logger.info('About to anayze file: {}'.format(filename))
    if not g.polygons:
        g.polygons.append({'name': 'full_image', 'value': [
                          (0, 0), (oldw, 0), (oldw, oldh), (0, oldh)]})
        g.logger.debug(
    ...
    if g.config['resize']:
        g.logger.debug(
            'resizing to {} before analysis...'.format(
                g.config['resize']))
        image = imutils.resize(image, width=min(
            int(g.config['resize']), image.shape[1]))
        newh, neww = image.shape[:2]
        # we resize polys only one time
        # when we get to the next image (snapshot), polygons have already
        # resized
        if not poly_scaled:
    ...
    g.logger.info(
        'Analyzing image with pattern: {}'.format(
    ...
    if len(bbox) == 0:
        g.logger.debug(
            'No patterns found using any models in {}'.format(filename))
        ...
        if g.config['write_image_to_zm'] == 'yes':
            if (args['eventpath']):
                g.logger.debug(
                    'Writing detected image to {}'.format(
                        args['eventpath']))
...
if (args['time']):
    g.logger.debug('detection took: {}s'.format(
